...
client_open_ai = OpenAI(
    api_key=os.getenv("API_PROXY_KEY"), base_url="https://api.proxyapi.ru/openai/v1"
)
        ...
        messages=[
            {
                ...
                "content": [
                    {
                        "type": "text",
                        "text": "You are an cardiologist. Describe the image and provide commentart on Russian Language.",
                    },
                    {
                        "type": "image_url",
                        "image_url": {
        ...
        if messages:
            for msg in messages:
                ...
                presigned_url = s3.generate_presigned_url(
                    ...
                    Params={"Bucket": os.getenv("BUCKET_NAME"), "Key": msg["Body"]},
                ...
                if response.status_code == 200:
                    ...
                    print(
                        f"Не удалось загрузить архив. Код состояния: {response.status_code}"
                    )
                ...
                s3_url = s3.generate_presigned_url(
                    ClientMethod="get_object",
                    Params={"Bucket": os.getenv("BUCKET_NAME"), "Key": object_name},
                    ExpiresIn=3600,  # Срок действия ссылки в секундах (1 час)
                )
...
def waiting_answers(file_name):
    ...
    while True:
        ...
        presigned_url = s3.generate_presigned_url(
            ClientMethod="get_object",
            Params={"Bucket": os.getenv("BUCKET_NAME"), "Key": file_name_result},
            ExpiresIn=3600,  # Срок действия ссылки в секундах (1 час)
        )
        response = requests.head(
            presigned_url
        )  # Используем HEAD-запрос для проверки доступности ссылки
        if response.status_code == 200:
            break
    return True
...
s3 = boto3_session.client("s3", endpoint_url=os.getenv("ENDPOINT"))
    ...
    if request.method == "POST":
        ...
        if "Item" in response:
            ...
            if check_password_hash(user["password_hash"], password):
                ...
                return redirect(
                    url_for("dashboard")
                )  # Переход на страницу личного аккаунта после входа
...
def dashboard():
    ...
    response = USERS.query(
        ...
        & Key("data_type").begins_with(f"result_users_data_ekg_{username}")
    )
    ...
    return render_template(
        ...
        has_ecg_data=has_ecg_data,
        selected_ecg_data=selected_ecg_data,
        has_selected_ecg_data=has_selected_ecg_data,
    )
...
def upload_to_s3(file, username_session):
    ...
    filename = secure_filename(
        username_session + "_" + f'{datetime.now().strftime("%Y-%m-%d-%H:%M:%S")}.zip'
    )
